...
for ($index = 1; $index <= $num_of_uploaded; $index++  ) {
    ...
    $statement = $pdo->prepare('INSERT INTO files (img_name, img_description, file_name) VALUES (:img_name, :img_description, :file_name)');

    $statement->execute(['img_name' => $img_name, 'img_description' => $img_description, 'file_name' => $_FILES[$fileName]['name']]);
}
